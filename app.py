...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://advocacyalawfrim.in", "http://localhost:3000"],
...
async def analyze_get():
    return JSONResponse({
        "message": "Please use POST /analyze with JSON body (text or file_url).",
...
async def analyze_contract(payload: ContractAnalysisRequest):
    ...
    if not text and payload.file_url:
        text = extract_text_from_pdf(payload.file_url)
    if not text:
        raise HTTPException(400, "No text provided")
    prompt = f"""You are an Indian contract law expert. Analyze the contract clause-by-clause. Return JSON:
{{
  "summary": "...",
  "clauses": [{{"clause_name": "...", "risk": "High/Medium/Low", "legal_basis": "...", "suggestion": "..."}}],
  "missing_clauses": ["..."],
  "lawyer_review": null
}}
...
JSON:"""
    ...
    try:
        ...
        result = {"summary": "Parsing error", "clauses": [], "missing_clauses": [], "lawyer_review": None}
