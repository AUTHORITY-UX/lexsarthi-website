...
# ===================================================================
# AGENTS LIST - 50+ AGENTS (FULLY FIXED)
# ===================================================================
AGENTS = [
    # Contract Review (8)
...
async def launch_campaign(
    ...
    conn = get_db()
    conn.execute(
        "UPDATE campaigns SET status = 'active', sent_date = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?",
        (campaign_id, current_user["id"])
    )
    conn.commit()
    conn.close()
    return {"message": "Campaign launched successfully", "website": WEBSITE_URL}

@app.get("/campaigns/performance")
async def get_campaign_performance(current_user: dict = Depends(get_current_user_bearer)):
    conn = get_db()
    campaigns = conn.execute(
        """SELECT id, name, type, status, open_count, click_count, response_count, sent_date 
           FROM campaigns WHERE user_id = ? AND status = 'active'""",
        (current_user["id"],)
    ).fetchall()
    conn.close()
    return {
...
@app.get("/campaigns-features")
async def get_campaigns_features():
    return {
        "features": {
            "email_campaigns": {
                "name": "Email Campaigns",
                "description": "Automated legal newsletters, client updates, and compliance alerts.",
                "status": "active"
            },
            "client_engagement": {
                "name": "Client Engagement",
                "description": "AI-powered client communication, follow-ups, and meeting scheduling.",
                "status": "active"
            },
            "legal_alerts": {
                "name": "Legal Alerts",
                "description": "Real-time regulatory updates and case law alerts for clients.",
                "status": "active"
            },
            "market_intelligence": {
                "name": "Market Intelligence",
                "description": "Legal trend analysis and competitive intelligence reports.",
                "status": "active"
            }
        },
        "pilot_mode": "Live",
...
# ===================================================================
# DOMAIN AGREEMENT ENDPOINT - ₹2 DOMAIN AGREEMENT
# ===================================================================
@app.post("/domain-agreement")
async def generate_domain_agreement(
    request: DomainAgreementRequest,
    ...
    try:
        ...
        ssl_data = {"valid": False, "error": "SSL check failed"}
    ...
    try:
        ...
        dns_data = {"error": "DNS lookup failed"}
        ...
        for platform, url in platforms.items():
            try:
                async with httpx.AsyncClient(timeout=10, follow_redirects=False) as client:
                    ...
                    social_data[platform] = resp.status_code < 400
            except:
                social_data[platform] = False
    
    domain_agreement = {
        "domain": domain,
        "agreement_type": "Domain Purchase & Transfer Agreement",
        "date": datetime.now().strftime("%d %B %Y"),
        "parties": {
            "seller": {
                "name": "Domain Owner",
                "address": "To be verified via WHOIS"
            },
            "buyer": {
                "name": current_user.get("full_name", "Buyer") if current_user else "Buyer",
                "email": current_user.get("username", "") if current_user else "",
        ...
        "technical_analysis": {
            ...
            "traffic_estimate": "Estimated from SimilarWeb" if SIMILARWEB_API_KEY else "Not available",
            "social_media": social_data
        },
        "legal_references": {
            "it_act_2000": "Section 43A - Data Protection",
            "dpdp_act_2023": "Section 9 - Security Safeguards",
            "indian_contract_act": "Section 10 - Valid Contract"
        },
        "price": {
            "amount": 200,
            "currency": "INR",
            "amount_label": "₹2",
            "description": "Domain Agreement & Due Diligence Report"
        },
        "terms": [
            "This agreement is for the transfer of domain ownership",
            "Due diligence has been performed using AI-assisted tools",
            "Buyer to verify all information independently",
            "Seller confirms domain is free of any legal disputes",
            "Transfer will be completed within 7 working days",
            "All data processed with Zero Retention policy",
        ...
        "lawyer_review": {
            ...
            "certifications": ["DPDP Act 2023 Compliance", "GDPR Certified"]
        },
        "disclaimer": "AI-assisted domain due diligence - verify with a licensed advocate before signing"
    }
    ...
    if current_user:
        ...
        conn.execute(
            ...
            (current_user["id"], "domain_agreement", f"Domain: {domain}", json.dumps(domain_agreement))
        )
        conn.commit()
        conn.close()
    
    return JSONResponse(domain_agreement)
