...
Fixes: "JWT is coded but not enforced". Now JWT tokens are verified on
every protected endpoint. Admin endpoints require ADMIN_KEY or ADMIN_SECRET.
Rate limiting is enforced via Redis (100 req/min per IP by default),
falling back to in-memory if Redis is down.
"""
