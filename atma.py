# =============================================================================
# atma.py – ATMA Router with Jury Verification
...
class AtmaRouter:
    ...
    multi‑LLM fallback, jury of 3 verifiers, and Judge Shakti.
    ...
    async def run(self, query: str, history: Optional[List[Dict]] = None, files: Optional[List] = None, unrestricted: bool = False) -> Dict:
        ...
        # --- DEFENSIVE CONTEXT BUILDING (FIX) ---
        context_parts = []
        for c in chunks:
...
"""
        ...
        provider = "groq"
        initial_answer = await self.call_llm(system_prompt, query, provider=provider)
...
Return JSON only: {{"status": "...", "confidence": "...", "corrected_text": "..."}}"""
            try:
                result_json = await self.call_llm(v_prompt, "Please verify.", provider=provider)
                try:
...
"""
        try:
            judge_output = await self.call_llm(judge_prompt, "Deliver final judgment.", provider=provider)
            try:
